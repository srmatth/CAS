# This script reads in the data and summarizes it

# Load the libraries and set up the spark connection
library(sparklyr)
library(dplyr)
library(tictoc)
library(BBmisc)
...
# Set which files we will be working with
...
# Read in the data and bind it all together in one
...
purrr::map(
  .x = files,
  .f = ~{
    usethis::ui_info("Copying file {.x} to Spark Cluster...")
    tmp_data <- readr::read_rds(.x)
    bi_tmp <- copy_to(sc, tmp_data)
    all_bi <- all_bi %>%
      bind_rows(bi_tmp)
    usethis::ui_done("Bound to overall data frame!")
    rm(tmp_data)
  }
)
