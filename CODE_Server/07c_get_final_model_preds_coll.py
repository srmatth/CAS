# Training of the best models for the COLL data
import logging
import numpy as np
import pandas as pd
...
logging.basicConfig(format='%(asctime)s - %(message)s', level=logging.INFO, filename="07a_out.log")
logging.info("libraries imported!")

train = pd.read_csv("data/coll_train.csv", low_memory=False)
train = train.dropna(axis=0)
test = pd.read_csv("data/coll_test.csv", low_memory=False)
...
# These are set manually, by me based on the 06a file in CODE_Analysis
...
X_test.to_csv("output/coll_final_mod_test_data.csv")
...
freq_preds.to_csv("output/coll_freq_preds_final_mod.csv")
...
p0_shap.to_csv("output/coll_freq_shap_values_final_mod_0.csv")
p1_shap.to_csv("output/coll_freq_shap_values_final_mod_1.csv")
p2_shap.to_csv("output/coll_freq_shap_values_final_mod_2.csv")
p3_shap.to_csv("output/coll_freq_shap_values_final_mod_3.csv")
...
# These parameters are pulled in manually here
sev_mod = sk.GradientBoostingRegressor(loss = "huber", n_estimators=1000, max_depth=1, learning_rate = 0.001,min_impurity_decrease=.0001)
...
sev_preds.to_csv("output/coll_sev_preds_final_mod.csv")
...
sev_shap = sev_ex.shap_values(X_test)
logging.info("shap values calculated")

sev_shap = pd.DataFrame(sev_shap)
logging.info("saving severity shap values")
sev_shap.to_csv("output/coll_sev_shap_values_final_mod.csv")
