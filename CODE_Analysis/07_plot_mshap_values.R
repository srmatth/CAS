...
dat_dir <- stringr::str_c("output/", dataset, "_final/")
...
observation_plot(
  ...
  shap_values = sev_shap[8,],
  expected_value = mean(sev_preds$`0`),
  num_vars = 10
)
...
freq_ex <- c(0.19150539, 0.26197266, 0.25506165, 0.29146029)
sev_ex <- 2478.81781705
...
p1 <- observation_plot(
  ...
  num_vars = 10,
  title = "Prediction Less than Baseline"
)

p2 <- observation_plot(
  variable_values = test[116,],
  shap_values = ev_explained[116,],
  expected_value = shap_expected_values,
  num_vars = 10,
  title = "Prediction Greater than Baseline"
)

p3 <- observation_plot(
  variable_values = test[548,],
  shap_values = ev_explained[548,],
  expected_value = shap_expected_values,
  num_vars = 10,
  title = "Prediction Near Baseline"
)

plt <- p1 | p2 | p3 

plt + plot_annotation(
  title = "3 Individual Prediction Explanations",
  theme = theme(
